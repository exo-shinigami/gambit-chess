...
int MakeMove(S_BOARD *pos, int move) {
	...
	if(captured != EMPTY) {
        ...
        if(captured >= wP && captured <= wK) {
            // White piece captured
            if(pos->capturedWhiteCount < 16) {
                pos->capturedWhite[pos->capturedWhiteCount++] = captured;
            }
        } else if(captured >= bP && captured <= bK) {
            // Black piece captured
            if(pos->capturedBlackCount < 16) {
                pos->capturedBlack[pos->capturedBlackCount++] = captured;
            }
        }
    }
...
void TakeMove(S_BOARD *pos) {
    ...
    if(captured != EMPTY) {
        ...
        if(captured >= wP && captured <= wK) {
            ...
            if(pos->capturedWhiteCount > 0) {
                pos->capturedWhiteCount--;
            }
        } else if(captured >= bP && captured <= bK) {
            // Black piece was captured, remove it from list
            if(pos->capturedBlackCount > 0) {
                pos->capturedBlackCount--;
            }
        }
    }
